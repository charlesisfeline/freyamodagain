...
class FuckState extends FlxUIState
{
  ...
  @:keep inline public static function FUCK(e:Dynamic, ?info:String = "unknown", _forced:Bool = false, _FATAL:Bool = false, _rawError:Bool = false)
  {
    ...
    var exception = "Never mind, you get no expection, idiot.";
      ...
      try
      {
        ...
        catch (e)
        {
          ...
          try
          {
            ...
            // err +='\n Working Path: ${KSELoader.absolutePath('')}';
        ...
        sys.io.File.saveContent('crashReports/KITSUNE_VSLICE_CRASH-${dateNow}.log', err);

        saved = true;
        trace('Wrote a crash report to ./crashReports/KITSUNE_VSLICE_CRASH-${dateNow}.log!');
        trace('Crash Report:\n$err');
      }
      catch (e)
      {
      ...
      try
      {
        if (!showingError)
        {
          ...
          if (saved)
          {
            ...
            textFieldBot.text = 'Saved crash report to "crashReports/KITSUNE_VSLICE_CRASH-${dateNow}.log".\nPlease send this file when reporting this crash.';
          }
        ...
        // Main.instance.addChild(new kse.ErrorSprite('${exception}\nThis happened in ${info}',saved));
      }
      catch (e)
      {
        trace('FUCK $e');
  ...
  override function create()
  {
    ...
    if (saved)
    {
      ...
      txt.text = 'Crash report saved to "crashReports/KITSUNE_VSLICE_CRASH-${dateNow}.log".\n ' + txt.text.substring(41);
    }
    useOpenFL = true;
  }
